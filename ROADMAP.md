...
- [ ] **Tab List Synchronization after Revive**: likely *not* a vanilla client cache problem. `PlayerEvent.TabListNameFormat` is fired from `ServerPlayer.refreshTabListName()`, which caches the result in `tabListDisplayName`; `ScoreboardHandler.refreshPlayerTabList` only broadcasts `ClientboundPlayerInfoUpdatePacket(UPDATE_DISPLAY_NAME, ...)`, which re-serialises that **cached** value, so the `[DEAD]` / `[AFK]` tag never recomputes. Next step: confirm `ServerPlayer.refreshTabListName()` is accessible in NeoForge 21.1.249 and call it before (or instead of) the manual broadcast. Left unchanged for now because it could not be compile-verified.
- [ ] **Preserve foreign scoreboard teams**: `ScoreboardHandler.updatePlayerScoreboard` puts players on `inrp_active` / `inrp_afk`, and vanilla's `addPlayerToTeam` evicts them from whatever team they were on. A server using teams for rank prefixes (LuckPerms, datapacks, manual `/team join`) loses that membership permanently once the player leaves RP mode. Either remember and restore the previous team, or skip the team swap for players who already belong to a foreign team.
